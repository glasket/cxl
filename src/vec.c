...
XResult xvec_get(const XVec *const vec, usize idx) {
  ...
  assert(vec->len <= xbuf_cap(vec->buf));
  return xres_ok(xres_unwrap_unchecked_byte_ptr(xbuf_access(vec->buf, idx)));
}
...
XErr xvec_set(const XVec *const vec, const usize idx, const void *const val) {
  ...
  void *mem = xres_unwrap_unchecked_byte_ptr(xbuf_access(vec->buf, idx));
...
XErr xvec_push(XVec *const vec, const void *const val) {
  ...
  void *mem = xres_unwrap_unchecked_byte_ptr(xbuf_access(vec->buf, vec->len));
  memcpy(mem, val, xbuf_alignment(vec->buf));
  vec->len += 1;
  return X_OK;
}
...
XResult xvec_pop(XVec *const vec) {
  ...
  vec->len -= 1;
  return xres_ok(xres_unwrap_unchecked_byte_ptr(xbuf_access(vec->buf, vec->len)));
}
...
XErr xvec_insert(XVec *const vec, const usize idx, const void *const val) {
  ...
  // Don't need to check for success, len is always <= cap
  void *mem = xres_unwrap_unchecked_byte_ptr(xbuf_access(vec->buf, idx));
  memmove((byte *)mem + xbuf_alignment(vec->buf), mem, (vec->len - idx) * xbuf_alignment(vec->buf));
  memcpy(mem, val, xbuf_alignment(vec->buf));
  vec->len += 1;
  return X_OK;
}
...
XErr xvec_append(XVec *const vec, const XVec other) {
  ...
  return xvec_append_raw(vec, xopt_unwrap_byte_ptr(xbuf_ptr(other.buf)), other.len);
}
...
XErr xvec_append_raw(XVec *const vec, const void *const data, const usize data_elem_count) {
  ...
  memcpy(xopt_unwrap_byte_ptr(xbuf_ptr(vec->buf)) + vec->len, data, data_elem_count * xbuf_alignment(vec->buf));
